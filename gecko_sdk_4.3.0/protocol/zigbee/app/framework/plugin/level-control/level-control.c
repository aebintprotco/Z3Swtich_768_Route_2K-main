...
//  emberAfLevelControlClusterPrint(" test 1 \n ");
  updateCoupledColorTemp(endpoint);

//  emberAfLevelControlClusterPrint(" test 2 \n ");
...
//  emberAfLevelControlClusterPrint(" test 3 \n ");
  // Are we at the requested level?
  if (currentLevel == state->moveToLevel) {

   //   emberAfLevelControlClusterPrint(" test 4 \n ");
    if (state->commandId == ZCL_MOVE_TO_LEVEL_WITH_ON_OFF_COMMAND_ID
        ...
        emberAfLevelControlClusterPrint(" currentLevel 5  =%d  %d  %d\n ",(currentLevel != MIN_LEVEL),currentLevel,MIN_LEVEL);
      setOnOffValue(endpoint, (currentLevel != MIN_LEVEL));
      if (currentLevel == MIN_LEVEL && state->useOnLevel) {

    //      emberAfLevelControlClusterPrint(" test 7 \n ");
        status = emberAfWriteServerAttribute(endpoint,
                                             ZCL_LEVEL_CONTROL_CLUSTER_ID,
                                             ZCL_CURRENT_LEVEL_ATTRIBUTE_ID,
     ...
     //     emberAfLevelControlClusterPrint(" test 8 \n ");
        }
      }
    } else {

   //     emberAfLevelControlClusterPrint(" test 6 \n ");
      if (state->storedLevel != INVALID_STORED_LEVEL) {
        uint8_t storedLevel8u = (uint8_t) state->storedLevel;

    //    emberAfLevelControlClusterPrint(" test 9 \n ");
        status = emberAfWriteServerAttribute(endpoint,
                                             ZCL_LEVEL_CONTROL_CLUSTER_ID,
                                             ZCL_CURRENT_LEVEL_ATTRIBUTE_ID,
      ...
      //    emberAfLevelControlClusterPrint(" test 10 \n ");
        }
      }
    }

   // emberAfLevelControlClusterPrint(" test 11 \n ");
    writeRemainingTime(endpoint, 0);
  } else {

    //  emberAfLevelControlClusterPrint(" test 12 \n ");
    writeRemainingTime(endpoint,
                       state->transitionTimeMs - state->elapsedTimeMs);
    schedule(endpoint, state->eventDurationMs);

 //   emberAfLevelControlClusterPrint(" test 13 \n ");
  }
}
...
static void setOnOffValue(uint8_t endpoint, bool onOff)
{
  if (emberAfContainsServer(endpoint, ZCL_ON_OFF_CLUSTER_ID)) {
    emberAfLevelControlClusterPrintln("llSetting on/off to %p due to level change",
                                      onOff ? "ON" : "OFF");
                               ...
                               uint16_t storedLevel)
{
  ...
  if (currentLevel <= state->moveToLevel) {
    if (commandId == ZCL_MOVE_TO_LEVEL_WITH_ON_OFF_COMMAND_ID) {
        ...
        emberAfLevelControlClusterPrint(" moveToLevelHandler  =%d  %d  %d\n ",(state->moveToLevel != MIN_LEVEL),state->moveToLevel,MIN_LEVEL);
...
static void moveHandler(uint8_t commandId, uint8_t moveMode, uint8_t rate, uint8_t optionMask, uint8_t optionOverride)
{
  ...
  if (currentLevel <= state->moveToLevel) {
    if (commandId == ZCL_MOVE_WITH_ON_OFF_COMMAND_ID) {
        ...
        emberAfLevelControlClusterPrint(" moveHandler  =%d  %d  %d\n ",(state->moveToLevel != MIN_LEVEL),state->moveToLevel,MIN_LEVEL);
        setOnOffValue(endpoint, (state->moveToLevel != MIN_LEVEL));
    }
    if (currentLevel == state->moveToLevel) {
      status = EMBER_ZCL_STATUS_SUCCESS;
                        ...
                        uint8_t optionOverride)
{
  ...
  if (currentLevel <= state->moveToLevel) {
    if (commandId == ZCL_STEP_WITH_ON_OFF_COMMAND_ID) {
        ...
        emberAfLevelControlClusterPrint(" stepHandler  =%d  %d  %d\n ",(state->moveToLevel != MIN_LEVEL),state->moveToLevel,MIN_LEVEL);
         setOnOffValue(endpoint, (state->moveToLevel != MIN_LEVEL));
    }
    if (currentLevel == state->moveToLevel) {
      status = EMBER_ZCL_STATUS_SUCCESS;
...
#if 1
  emberAfLevelControlClusterPrintln("pinbi  or guanlian  \n");    //这里是亮度被开关关联的部分
  if (newValue) {
