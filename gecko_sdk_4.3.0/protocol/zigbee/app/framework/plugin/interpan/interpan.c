...
void interpanPluginInit(uint8_t init_level)
{
  switch (init_level) {
    ...
    {
#if defined(ALLOW_FRAGMENTATION)
      sl_zigbee_event_init(&emberAfPluginInterpanFragmentTransmitEvent,
                           emberAfPluginInterpanFragmentTransmitEventHandler);
      sl_zigbee_event_init(&emberAfPluginInterpanFragmentReceiveEvent,
                           emberAfPluginInterpanFragmentReceiveEventHandler);
#endif // ALLOW_FRAGMENTATION
      break;
    }
    ...
    case SL_ZIGBEE_INIT_LEVEL_LOCAL_DATA:
    {
      ...
      for (i = 0; i < EMBER_AF_PLUGIN_INTERPAN_FRAGMENTATION_MAX_OUTGOING_PACKETS; i++) {
        ...
        txPackets[i].event = &emberAfPluginInterpanFragmentTransmitEvent;
      }
      for (i = 0; i < EMBER_AF_PLUGIN_INTERPAN_FRAGMENTATION_MAX_INCOMING_PACKETS; i++) {
        MEMSET(rxPackets[i].sourceEui, 0, EUI64_SIZE);
        rxPackets[i].event = &emberAfPluginInterpanFragmentReceiveEvent;
      }
#endif //ALLOW_FRAGMENTATION
      break;
    }
