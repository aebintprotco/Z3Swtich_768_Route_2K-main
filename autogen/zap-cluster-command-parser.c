...
#if (defined(SL_CATALOG_ZIGBEE_ZCL_FRAMEWORK_CORE_PRESENT) || defined(SL_CATALOG_ZIGBEE_AF_SUPPORT_PRESENT))
#include <stdint.h>
...
// Signature for ConfigureInterface command
static const sl_signature_t sig_zigbee_direct_configuration_cluster_configure_interface_command = sl_zcl_zigbee_direct_configuration_cluster_configure_interface_command_signature;
// Command parser for ConfigureInterface command
EmberAfStatus zcl_decode_zigbee_direct_configuration_cluster_configure_interface_command (EmberAfClusterCommand * cmd, sl_zcl_zigbee_direct_configuration_cluster_configure_interface_command_t *cmd_struct) {
  // Use signature-decoding mechanism to parse this command.
  return sli_do_decode(cmd, sig_zigbee_direct_configuration_cluster_configure_interface_command, (uint8_t*)cmd_struct);
} 
...
// Signature for ConfigureAnonymousJoinTimeout command
static const sl_signature_t sig_zigbee_direct_configuration_cluster_configure_anonymous_join_timeout_command = sl_zcl_zigbee_direct_configuration_cluster_configure_anonymous_join_timeout_command_signature;
// Command parser for ConfigureAnonymousJoinTimeout command
EmberAfStatus zcl_decode_zigbee_direct_configuration_cluster_configure_anonymous_join_timeout_command (EmberAfClusterCommand * cmd, sl_zcl_zigbee_direct_configuration_cluster_configure_anonymous_join_timeout_command_t *cmd_struct) {
  // Use signature-decoding mechanism to parse this command.
  return sli_do_decode(cmd, sig_zigbee_direct_configuration_cluster_configure_anonymous_join_timeout_command, (uint8_t*)cmd_struct);
} 
...
8u ||
 !((cmd_struct->options & 0x0007) == 0x0002)) 
    { 
        memset(cmd_struct->gpdIeee, 0, 8);
    } 
    else 
    {
        memcpy(cmd_struct->gpdIeee, sli_decode_bytes(cmd, payloadOffset), 8);
...
8u ||
 !((cmd_struct->options & 0x000010) == 0x000000 && ((cmd_struct->options & 0x000060) == 0x000000 || (cmd_struct->options & 0x000060) == 0x000060))) 
    { 
        memset(cmd_struct->sinkIeeeAddress, 0, 8);
    } 
    else 
    {
        memcpy(cmd_struct->sinkIeeeAddress, sli_decode_bytes(cmd, payloadOffset), 8);
...
16u ||
 !((cmd_struct->options & 0x008000) == 0x008000)) 
    { 
        memset(cmd_struct->gpdKey, 0, 16);
    } 
    else 
    {
        memcpy(cmd_struct->gpdKey, sli_decode_bytes(cmd, payloadOffset), 16);
...
16u ||
 !((cmd_struct->options & 0x0200))) 
    { 
        memset(cmd_struct->gpdSecurityKey, 0, 16);
    } 
    else 
    {
        memcpy(cmd_struct->gpdSecurityKey, sli_decode_bytes(cmd, payloadOffset), 16);
